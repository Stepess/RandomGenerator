...
public class UniformitySignsCriterion extends AbstractCriterion {
    ...
    public void calculateChiSqr() {
        for (int i = 0; i < R; i++) {
            for (int j = 0; j < NUM_OF_DIFFERENT_BYTES; j++) {
                double denominator = sumColumn(statisticData, i);
                if (denominator != 0) {
                    chiSqr += Math.pow(statisticData[i][j], 2) / ((denominator));
                }
            }
        }
        chiSqr /= M;
    ...
    private double sumColumn(int[][] array, int columnIndex) {
        double result = 0;
        for (int i = 0; i < R; i++) {
